...
///     달력 날짜) 그 복습을 재개한다. 오늘 이미 복습을 마쳤거나 건너뛴
///     적이 있으면(`SessionStateService.hasReviewedToday`) 그 즉시 (C)로
///     넘어간다 — 그 사이 새로 학습한 문장이 TTS 캐시까지 생겨 "복습
///     가능"해졌더라도 오늘 다시 복습으로 보내지 않기 위함이다. 그렇지
...
Future<String?> _resolveLearningEntryRoute({
  ...
  // 모른다 — 이 검사가 없으면, 복습을 마친 뒤 새 학습에서 문장을 한
  // turn만 완료해도(그 문장이 TTS까지 캐시되어 곧바로 복습 가능 상태가
  // 됨) 세션이 재평가되는 순간(예: 일일 turn 한도 도달, "학습 종료", 앱
  // 재시작) 방금 학습한 그 문장이 "새로 복습 가능해진 항목"으로 잡혀
