...
class ShadowingViewModel extends Notifier<ShadowingState>
  ...
  ///
  /// 호출한 쪽(ShadowingPronunciationScreen)이 `context.go(route)`로 이동해야
  /// 할 라우트 문자열을 반환한다 — 보통은 [AppRoutes.writing]이지만, 이
  /// 턴으로 방금 [kDailyTurnLimit]에 도달했다면 세션을 "학습 종료"와
  /// 똑같은 방식으로 자동 finalize한 뒤, 11번째 문장을 생성하려 시도하는
  /// 대신 곧바로 [AppRoutes.review]를 반환한다 — `/learning`을 거치지
  /// 않으므로 라우터의 일반 진입 분기(오늘 이미 복습을 마쳤으면 그냥 다음
  /// 학습을 또 시작하는 로직)를 타지 않고, 결과적으로 하루 한도가 걸리지
  /// 않고 계속 새 세션이 반복되던 버그를 막는다.
  Future<String> completeTurnAndAdvanceToWriting() async {
    final sessionService = ref.read(sessionStateServiceProvider);
    var session = await sessionService.readState();
    session ??= await sessionService.startNewSession(initialType: ExerciseType.shadowing);
